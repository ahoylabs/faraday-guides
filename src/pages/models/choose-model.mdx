...
**Models are downloaded directly to your hard drive.**

![Manager](/images/model_manager.png)

## File Formats

There are multiple Large Language Model (LLM) file formats. The three most common ones are FP16, GPTQ, GGUF, and GGML. FP16 is the original file created via training; GPTQ is a format used for inference fully on the GPU; GGML and GGUF are for running on the CPU.
...
GGUF/GGML model files are quantized, which means they are compressed to a smaller size than the original model. This allows them to run on lower-end hardware, such as consumer devices. Of course, there is a trade-off between quantization level and output quality. You can compare it to image compression, where reducing the size eventually results in pixelation.

A helpful way of visualizing the impact of quantization is to consider the range of possible responses to the question "Tell me about pizza".
...
For the pizza example, we might only need one answer: 'It's great', but for a more complex input, you will likely want the model to have more nuance (and thus, a different quantization level with more precision). This analogy is an oversimplification, but it's a good way to think about the trade-off between quantization and quality.

For inference (text-generation), the drop in quality does not scale linearly with the reduction in bits. There is very little difference in the quality of response from an 8-bit quantized model versus a full FP16 model. However, you will see a notable difference when compared to a 2-bit model.

### K-Quants

Faraday supports model files derived using the "K-quants" quantization method. The "Q4" describes the level of quantization, the K denotes that it’s a “k-quant” and the M denotes that it’s a medium size k-quant.

**GGUF files ending in “Q4_K_M” or “Q5_K_M” are currently recommended for most users.**
